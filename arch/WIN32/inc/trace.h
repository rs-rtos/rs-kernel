...
#endif
/*===========================================================================*/
typedef void* (*P_TR)(const char *, ...);

#define __ui_writec(s, count)   __write(s, count)

#if CFG_DEBUG > 0
#define RSI_TRACE           __trace("%s(%d): ",__FILE__,__LINE__)
#define RSI_ASSERT(x)       assert(x)
/*
#define RSI_ASSERT(x) \
    do { \
        if (!(x)) { \
...
void __dump(const char* file, int line);
P_TR __trace(const char *fmt, ...);
void __write(const char *s, int count);
